...
export class EnhancedTextPreprocessor {
    ...
    private semanticPatterns: SemanticPattern[] = [
        ...
        {
            pattern: /(you|u) (are|r) (no good|not good) (to|for) (the )?(world|society|anyone|anything|people|us)/i,
            meaning: "Worth attack - no good to the world/society",
            severity: 0.88,
            category: "worth_attack"
        },
        {
            pattern: /(you|u) (are|r) (of )?no (use|value|worth|benefit) (to|for) (the )?(world|society|anyone|anything|people)/i,
            meaning: "Worth attack - no use/value to world",
            severity: 0.87,
            category: "worth_attack"
        },
        {
...
export class CyberbullyDetector {
    ...
    private toxicPhrases: string[] = [
        ...
        "good for nothing", "no good to the world", "not good for anything",
        "no use to anyone", "no value to society", "of no use",
    ...
    public classify(text: string): ProcessedData {
        ...
        // Debug logs to help trace why some inputs are classified as safe
        try {
            console.log('[Semantic Detector] classify() input:', text);
            console.log('[Semantic Detector] normalized:', normalized);
            console.log('[Semantic Detector] tokens:', tokens.slice(0, 40));
            console.log('[Semantic Detector] semanticMatches:', semanticMatches.map(m => ({ meaning: m.meaning, severity: m.severity }))); 
        } catch (e) {
        ...
        if (criticalWordCount >= 1) {
            ...
            const hasPersonalContext = /\b(you|your|you're|youre|ur|u are)\b/i.test(text);
            if (hasPersonalContext) {
                // Don't auto-flag - let threshold logic decide (more accurate)
                severityBoost = 0.25; // Increased boost but not auto-flag
                matchedSignals.push(`LOW_SEVERITY_PERSONAL:${lowWordCount}`);
            } else {
                severityBoost = 0.08; // Reduced - not enough without personal context
            ...
            const hasPersonalContext = /\b(you|your|you're|youre|ur|u are)\b/i.test(text);
            severityBoost = hasPersonalContext ? 0.10 : 0.05; // Minimal boost
            matchedSignals.push(`LOW_SEVERITY_WORDS:${lowWordCount}`);
        }

        // Combine base score with severity-weighted score (reduced boost multiplier)
        let finalScore = Math.max(score, avgSeverityScore) + (severityBoost * 0.4);
        ...
        if (criticalWordCount >= 2 || (criticalWordCount >= 1 && highWordCount >= 1)) {
            ...
            finalScore = Math.max(finalScore, 0.90); // Slightly reduced for accuracy
            matchedSignals.push(`DANGEROUS_CYBERBULLYING:${criticalWordCount} critical, ${highWordCount} high`);
        } else if (highWordCount >= 2) {
            isDangerousCyberbullying = true;
            finalScore = Math.max(finalScore, 0.72); // Reduced from 0.75
        ...
        // IMPROVED THRESHOLD LOGIC - More accurate, context-aware
        // Consider semantic patterns, personal targeting, and text length
        const hasPersonalTargeting = /\b(you|your|you're|youre|ur|u are|u r)\b/i.test(text);
        const hasStrongSemanticMatch = semanticMatches.some(m => m.severity >= 0.85);
        const textLength = tokens.length;
        
        let threshold: number;
        
        if (autoFlagAsCyberbully) {
            // Critical/high severity words detected - low threshold
            threshold = 0.18;
        } else if (hasStrongSemanticMatch && hasPersonalTargeting) {
            // Strong semantic pattern + personal targeting = cyberbullying
            threshold = 0.30;
        } else if (semanticMatches.length >= 2 && hasPersonalTargeting) {
            // Multiple semantic patterns + personal = likely cyberbullying
            threshold = 0.35;
        } else if (mediumWordCount >= 1 && hasPersonalTargeting && textLength <= 8) {
            // Medium severity word in short personal attack
            threshold = 0.38;
        } else if (lowWordCount >= 2 && hasPersonalTargeting && textLength <= 6) {
            // Multiple low severity words in short personal text
            threshold = 0.40;
        } else if (semanticMatches.length > 0 && hasPersonalTargeting) {
            // Single semantic match with personal targeting
            threshold = 0.42;
        } else if (highSeverity || semanticMatches.length > 0) {
            // High severity or semantic patterns without personal targeting
            threshold = 0.48;
        } else {
            // Default - require strong evidence for non-personal text
            threshold = 0.55;
        }
