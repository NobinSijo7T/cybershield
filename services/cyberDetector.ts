...
class CyberDetectorService {
  ...
  async analyzeText(text: string): Promise<AnalysisResult> {
    ...
    try {
      ...
      // IMPROVED: More refined semantic boost with diminishing returns
      let semanticBoostMultiplier;
      if (semanticBoost > 0.85) {
        // Very high semantic score = strong signal
        semanticBoostMultiplier = 0.25;
      } else if (semanticBoost > 0.7) {
        semanticBoostMultiplier = 0.18;
      } else if (semanticBoost > 0.5) {
        semanticBoostMultiplier = 0.12;
      } else if (semanticBoost > 0.3) {
        semanticBoostMultiplier = 0.08;
      } else {
        // Low semantic score = minimal boost
        semanticBoostMultiplier = 0.05;
      }
      
      const weightedRisk = (maxCategory * 0.65) + (avgScore * 0.35) + (semanticBoost * semanticBoostMultiplier);
      const riskLevel = Math.min(100, Math.round(weightedRisk * 100));
      
      // IMPROVED: More accurate cyberbullying threshold
      // Consider both risk level and semantic confidence
      const isCyberbullying = (riskLevel > 55 && semanticBoost > 0.35) || semanticBoost > 0.65 || riskLevel > 70;

      // IMPROVED: More accurate confidence calculation
      let confidence = 0;
      if (isCyberbullying) {
        // Higher confidence when multiple categories are triggered
        const activeCategoriesCount = [
          analysis.toxicity > 0.15,
          analysis.threat > 0.15,
          analysis.insult > 0.15,
          analysis.identity_hate > 0.15
        ].filter(Boolean).length;

        // Confidence based on: max score, semantic match, and category agreement
        const baseConfidence = maxCategory * 0.75;
        const categoryBonus = activeCategoriesCount * 0.08;
        const semanticConfidenceBonus = semanticBoost > 0.7 ? 0.15 : (semanticBoost > 0.4 ? 0.08 : 0.03);
        
        confidence = Math.min(0.96, baseConfidence + categoryBonus + semanticConfidenceBonus);
      } else {
        // Confidence for safe content inversely related to borderline scores
        if (maxCategory < 0.05 && semanticBoost < 0.2) {
          confidence = 0.95; // Very confident it's safe
        } else if (maxCategory < 0.2 && semanticBoost < 0.35) {
          confidence = 0.88; // Confident it's safe
        } else {
          // Borderline case - lower confidence
          confidence = 0.75 - (maxCategory * 0.3) - (semanticBoost * 0.2);
          confidence = Math.max(0.55, confidence); // Floor confidence at 55%
        }
      }
